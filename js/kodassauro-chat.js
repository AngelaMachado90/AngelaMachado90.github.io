...
(() => {
  ...
  const KODASSAURO_CONFIG = {
    ...
    channels: {
      ...
      email: 'mailto:angelamachado02022@gmail.com?subject=Contato%20KoddaHub&body=',
    ...
    demoLinks: [
      { label: 'Demo Institucional', href: `${DEMO_PREFIX}demo-institucional.html` },
  ...
  function hydrateState(raw) {
    ...
    merged.leadDraft = {
      ...base.leadDraft,
      ...(raw.leadDraft && typeof raw.leadDraft === 'object' ? raw.leadDraft : {})
    };
    merged.notifCount = Number.isFinite(raw.notifCount) ? raw.notifCount : base.notifCount;
    merged.lastNotifiedAt = Number.isFinite(raw.lastNotifiedAt) ? raw.lastNotifiedAt : base.lastNotifiedAt;
  ...
  function init() {
    ...
    function resetChat() {
      ...
      state.step = 'idle';
      state.messages = [];
      state.leadDraft = { solution: '', deadline: '', budget: '', name: '', channel: '' };
      setBadge(0);
      uiHydrated = false;
      messagesEl.innerHTML = '';
      quickEl.innerHTML = '';
    ...
    function startOrcamentoFlow() {
      ...
      state.step = 'solution';
      state.leadDraft = { solution: '', deadline: '', budget: '', name: '', channel: '' };
      addMessage('bot', 'Perfeito. Para qual tipo de solucao voce quer orcamento?');
      setQuickOptions([
    ...
    function startDuvidasFlow() {
      state.mode = 'duvidas';
      state.step = 'idle';
      addMessage('bot', 'Manda sua duvida por aqui. Se preferir, escreva uma palavra-chave como **preco**, **prazo**, **seo** ou **suporte**.');
      setQuickOptions([
    ...
    function startPortfolioFlow() {
      ...
      const opts = KODASSAURO_CONFIG.demoLinks.map(d => ({ label: d.label, href: d.href }));
    ...
    function handleTopAction(action) {
      switch (action) {
        ...
        case 'scroll_solucoes':
          addMessage('bot', 'Ok. Vou te levar para a secao de solucoes. Se quiser, eu continuo por aqui.');
          closePanel();
        ...
        case 'open_whatsapp_short':
          ...
          openUrl(
            KODASSAURO_CONFIG.channels.whatsapp +
              encodeURIComponent('Ola! Vim pelo site da KoddaHub e quero falar com um vendedor.')
          );
          break;
        default:
          break;
    ...
    function handleOrcamentoInput(text) {
      ...
      if (state.step === 'solution') {
        ...
        state.step = 'deadline';
        addMessage('bot', 'Qual seu prazo ideal? Ex: "15 dias", "ate o fim do mes".');
        setQuickOptions([
      ...
      if (state.step === 'deadline') {
        ...
        state.step = 'budget';
        addMessage('bot', 'Você tem um orcamento estimado? (Pode ser uma faixa)');
        setQuickOptions([
    ...
    function handleContatoInput(text) {
      ...
      if (state.step === 'channel') {
        const shortMsg = 'Ola! Vim pelo site da KoddaHub e quero falar com um vendedor.';
        if (t.includes('whatsapp')) {
          addMessage('bot', 'Perfeito. Vou abrir o WhatsApp agora.');
          openUrl(KODASSAURO_CONFIG.channels.whatsapp + encodeURIComponent(shortMsg));
          return;
        }
        if (t.includes('email') || t.includes('e-mail')) {
          addMessage('bot', 'Certo. Vou abrir um e-mail pronto.');
          openUrl(KODASSAURO_CONFIG.channels.email + encodeURIComponent(shortMsg));
          return;
        }
        if (t.includes('telefone') || t.includes('lig') || t.includes('call')) {
          addMessage('bot', 'Ok. Vou iniciar a ligacao.');
          window.location.href = KODASSAURO_CONFIG.channels.telefone;
          return;
        }

        addMessage('bot', 'Sem problema. Escolha um canal: WhatsApp, E-mail ou Ligacao.');
      }
    }
    ...
    function handleUserText(text) {
      ...
      if (t.includes('contato') || t.includes('vendedor') || t.includes('whatsapp')) {
        startContatoFlow();
        return;
      }
      if (t.includes('demo') || t.includes('portfolio') || t.includes('portifolio')) {
        startPortfolioFlow();
        return;
      }
    ...
    quickEl.addEventListener('click', (e) => {
      ...
      if (action) {
        ...
        if (action === 'open_whatsapp_orcamento') {
          ...
          addMessage('bot', 'Pronto. Se preferir, posso continuar por aqui tambem.');
          setDefaultQuick();
          return;
        }
