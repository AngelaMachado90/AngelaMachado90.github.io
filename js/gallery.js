...
(function () {
  ...
  function initializeCards() {
    cards.forEach((card) => {
      ...
      card.setAttribute(
        "aria-label",
        `Ver demonstração: ${card.querySelector("h4")?.textContent || "Template"}`,
      );
    });
  }
    ...
    if (cart) {
      ...
      addBtns.forEach((btn) => {
        btn.addEventListener("click", function () {
          ...
          const productName =
            product.querySelector("h5")?.textContent || "Produto";
  ...
  function simulateBooking(root) {
    ...
    bookBtns.forEach((btn) => {
      btn.addEventListener("click", function () {
        const service =
          this.closest(".service")?.querySelector("h4")?.textContent ||
          "serviço";
  ...
  function addButtonEffects(root) {
    root
      .querySelectorAll(".cta-sim, .open-demo, button:not(.no-effect)")
      .forEach((btn) => {
        btn.addEventListener("click", function () {
          // Efeito de clique
          this.animate(
